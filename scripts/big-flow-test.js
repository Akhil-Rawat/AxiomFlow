...
async function main() {
  ...
  const yieldEngine = await hre.ethers.getContractAt("YieldEngine", deployment.yieldEngine);
  ...
  console.log("✅ Approved", hre.ethers.formatEther(collateralAmount), "RWA tokens");

  // Step 2: Deposit collateral
  console.log("\n2️⃣ DEPOSITING COLLATERAL...");
  const depositTx = await lendingVault.depositCollateral(collateralAmount);
  await depositTx.wait();
  console.log("✅ Deposited", hre.ethers.formatEther(collateralAmount), "RWA as collateral");

  let position = await lendingVault.positions(user.address);
  console.log("   📦 Collateral:", hre.ethers.formatEther(position.collateralAmount), "RWA");
  ...
  console.log("   💵 Total Borrowed (All Time):", hre.ethers.formatEther(position.borrowedAmount), "MNT");
  console.log("   ⚠️  Current Debt:", hre.ethers.formatEther(position.remainingDebt), "MNT");
  ...
  await fundTx.wait();
  console.log("✅ Funded YieldEngine with", hre.ethers.formatEther(yieldAmount), "MNT");

  let yieldBalance = await hre.ethers.provider.getBalance(deployment.yieldEngine);
  console.log("   💎 YieldEngine balance:", hre.ethers.formatEther(yieldBalance), "MNT");

  // Step 5: Generate yield and reduce debt
  console.log("\n5️⃣ GENERATING YIELD & REDUCING DEBT...");
  const debtBefore = position.remainingDebt;
  console.log("   💸 Debt BEFORE yield:", hre.ethers.formatEther(debtBefore), "MNT");
  ...
  console.log("   ✨ Debt AFTER yield:", hre.ethers.formatEther(debtAfter), "MNT");
  
  const totalDebtPaid = position.borrowedAmount - position.remainingDebt;
  const justPaid = debtBefore - debtAfter;
  console.log("   💰 Just Paid by Yield:", hre.ethers.formatEther(justPaid), "MNT");
  console.log("   🎉 Total Debt Paid (All Time):", hre.ethers.formatEther(totalDebtPaid), "MNT");

  // Final state
  console.log("\n📊 FINAL POSITION SUMMARY");
  console.log("=" .repeat(70));
  console.log("   📦 Collateral Locked:", hre.ethers.formatEther(position.collateralAmount), "RWA");
  console.log("   💵 Total Borrowed:", hre.ethers.formatEther(position.borrowedAmount), "MNT");
  console.log("   ⚠️  Remaining Debt:", hre.ethers.formatEther(position.remainingDebt), "MNT");
  console.log("   ✅ Paid by Yield:", hre.ethers.formatEther(totalDebtPaid), "MNT");
  
  const percentagePaid = (Number(totalDebtPaid) / Number(position.borrowedAmount)) * 100;
  console.log("   📊 Debt Reduction:", percentagePaid.toFixed(2) + "%");

  let totalDebt = await lendingVault.totalDebt();
  console.log("\n   🌐 Total Platform Debt:", hre.ethers.formatEther(totalDebt), "MNT");
  
  let vaultBalance = await hre.ethers.provider.getBalance(deployment.lendingVault);
  console.log("   🏦 Vault MNT Balance:", hre.ethers.formatEther(vaultBalance), "MNT");
